...
| C2 | Prefix-limited value is rare: 1/178 survives taxonomy (3 non-instant advantage crossings, 2 already restart-solvable in-grid; 4 instant-advantaged) | a5_labels_project/a5_full_cohort_labels.parquet + summary | 1 prefix-limited (gemma math_03159, T_V=896, adv 0.75); count reported descriptively — 89 problems repeat across both models so cells are not independent, no prevalence CI in paper (CP [0.014%, 3.09%] computed, not reported); δ=0.25→7, δ=0.75→2; conservative agrees 178/178; cohort outcome-blind but difficulty-targeted, one base trajectory per cell | a5_project_regime_map.png |
