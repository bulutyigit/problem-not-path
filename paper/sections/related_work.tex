...
finding that commitment rises steadily with prefix length. All of
...
compute, which converts per-anchor solve rates into a value measurement and
...
remain genuinely stochastic (success 0.3--0.6 at $n{=}8$), which undermines
...
units. The answer, for our models, is compression rather than reachability
(prefix wins 11/13 at equal compute; an 8{,}192-token restart catches up in
11/13), which is empirical evidence for the no-benefit branch of the dichotomy of
...
where it bites: over forecast windows of 128--2{,}048 tokens, frozen
summary features of the run's internal dynamics add no AUROC over a
difficulty baseline on held-out problems --- consistent with reports that
...
difficulty axis has remained open --- notably, no reviewer raised a
question-only baseline in the public review file of
\citet{zhang2025knowright}, whose decision even lists the unexplained
predictiveness of hidden states as a weakness. Meanwhile, difficulty
