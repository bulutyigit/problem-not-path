...
missing controls mirror each other. Breakthrough claims rest on
...
control at matched total compute, which no published probe runs.
...
published positives do not report; the confound was not raised even in the
public review file of the area's flagship result.
...
compute, splitting the naive breakthrough time into a budget-fit time
...
cells shows genuine prefix-locked value; 98 cells collected after the
...
compute-starved); the other's do not budge (capability-limited).
\item \textbf{Accumulated reasoning is compute compression.} At matched
total compute the model's own prefix beats restarting (11/13), yet a
larger restart budget catches up (11/13): long reasoning reaches the same
places cheaper, not new places.
\item \textbf{Solvability is not binary.} A third of ambiguous
intermediate states remain genuinely stochastic (success $0.3$--$0.6$ at
eight attempts).
\item \textbf{Early internal signals carry no outcome information beyond
difficulty.} A pre-registered, single-shot test on a held-out split finds
$\Delta$AUROC $\approx 0$ at every forecast window
(Section~\ref{sec:results-prediction}); on public data, a question-only
signal reaches 0.873 on 192K DeepSeek-R1 generations --- inside the
published probe range --- and the closest published early-window positive
replicates at $t{=}4$ (pooled 0.849) while sitting exactly at chance
(0.496) within problem, at every anchor.
\end{itemize}

The claims are scoped deliberately. We study two small non-reasoning-tuned
models plus public corpora from one reasoning-tuned family; we do not claim
internal states never carry within-attempt information --- at
intermediate-answer positions, in larger models, or in other domains the
verdict may differ. What we do claim is that the two controls change
...
claims about reasoning should not be published without them. Every
...
(Section~\ref{sec:method}, App.~\ref{app:amendments}).
