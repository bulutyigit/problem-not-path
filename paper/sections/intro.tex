...
total compute the model's own prefix beats restarting wherever the
comparison is exactly matched (9/9, with 2 wins and 2 ties on boundary
proxies), yet a larger restart budget reaches the same threshold in 11/13
and the prefix's own rate in 9/13: long reasoning reaches the same places
cheaper, not new places.
...
information beyond difficulty.} A pre-registered, single-shot test on a held-out split finds
