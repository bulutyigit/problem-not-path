...
was pre-registered; an equivalence-style reading is licensed only where
the intervals are tight, in the within-problem dissection below.
...
on both endpoints) and mediocre for Ministral-3 ($0.69$--$0.73$).
Difficulty almost fully accounts for the capability-limited model's
outcomes, while the
...
trajectories, evaluated within problem --- the within component of
...
(Figure~\ref{fig:dissect}). The entire positive is problem identity, far
below the dump's full leave-one-out difficulty ceiling of $0.981$
(pass rate of the 255 other attempts per problem). Moreover, with sample composition held fixed, the pooled AUROC
