...
compute-starved model, abandoning a long prefix costs roughly a budget
multiple, not access to the solution; for a capability-limited model,
neither continuation nor restart helps. Which regime a deployment faces is
measurable with a four-point restart curve.

\paragraph{What the prediction results do and do not kill.}
Three layers --- a pre-registered null on our cohort, a question-only
ceiling inside the published range on 192K public R1 generations, and a
...
immediate or absent. Positive existence claims (the one prefix-limited
cell, the stochastic band) rest on modest counts and warrant replication.

\paragraph{Future work.}
The natural next experiment runs the same frozen instrument on a
reasoning-tuned model, where aha-moment claims actually live: interior
