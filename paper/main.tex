...
from-scratch restart curves $R(C)$ at matched \emph{total}
generated-token budget, with
interval censoring and attempt-level noise handled explicitly. Applying the
instrument to 178 problem--model cells from an outcome-blind,
difficulty-targeted cohort (89 MATH problems $\times$ two small
...
(flat across the measured budgets); (iii) wherever the matched total
token budget lies inside the restart grid,
...
rate in 9/13 --- predominantly consistent with accumulated reasoning
acting as \emph{compute compression} rather than expanded reachability,
within the measured budget range; (iv) roughly a third of ambiguous
...
beyond a problem-difficulty baseline --- with a labeled post-hoc sweep
finding no alternative forecast point from 128 to 2,048 tokens whose
interval excludes zero --- a control that the growing literature of
probe-based positives lacks. Two generation-free analyses of public corpora show the
...
replicates (pooled 0.849 at $t{=}4$) while statistically
indistinguishable from chance \emph{within problem} at every anchor, on
the 22 non-saturated problems
